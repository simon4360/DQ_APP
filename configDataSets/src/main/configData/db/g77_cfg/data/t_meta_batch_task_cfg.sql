...
Prompt drop and create empty configuration table for g77_cfg.t_meta_batch_task_cfg
...
-- Populate G77_CFG.T_META_BATCH_TASK_CFG
Prompt Populate G77_CFG.T_META_BATCH_TASK_CFG
INSERT INTO G77_CFG.T_META_BATCH_TASK_CFG ("PROCESS_TYPE","PROCESS_NAME","PROCESS_DESCRIPTION","ERROR_THRESHOLD","THRESHOLD_IS_PERCENT","RUN_CONTROL","ACTIVE_INDICATOR","INTERFACE_ID")
...
delete from g77_cfg.t_meta_batch_task target
where not exists (select 1 from g77_cfg.t_meta_batch_task_cfg cfg where target.INTERFACE_ID = cfg.INTERFACE_ID and target.PROCESS_NAME = cfg.PROCESS_NAME);

merge
 into
   g77_cfg.t_meta_batch_task target
   using
     ( select   
            cfg.interface_id,
            cfg.process_type,
         ...
         from g77_cfg.t_meta_batch_task_cfg cfg
         inner join g77_cfg.t_meta_batch_task target
        ...
        union all
         select   
            ...
            cfg.active_indicator
         from g77_cfg.t_meta_batch_task_cfg cfg
         where not exists (select 1 from g77_cfg.t_meta_batch_task target where target.interface_id = cfg.interface_id and target.process_name = cfg.process_name )
         ) cfg
  on ( target.interface_id = cfg.interface_id and target.process_name = cfg.process_name  )
  when matched 
  then 
...
Prompt dropping temporary config table g77_cfg.t_meta_batch_task_cfg

begin
  execute immediate 'drop table g77_cfg.t_meta_batch_task_cfg';
end;
/
