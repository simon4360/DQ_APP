...
Prompt drop and create empty configuration table for g77_cfg.T_META_INTERFACE_CFG
...
-- Populate G77_CFG.T_META_INTERFACE_CFG
Prompt Populate G77_CFG.T_META_INTERFACE_CFG
INSERT INTO G77_CFG.T_META_INTERFACE_CFG ("TECHNOLOGY","APTITUDE_PROJECT","APTITUDE_PROJECT_DESC","COUNTERPARTY_SYSTEM","COUNTERPARTY_ROLE","STOP_PROJECT_FLAG","RESET_EVENTS_FLAG","IS_ACTIVE")
...
delete from g77_cfg.t_meta_interface target
where not exists (select 1 from g77_cfg.t_meta_interface_cfg cfg where target.aptitude_project = cfg.aptitude_project);

merge
 into
   g77_cfg.t_meta_interface target
   using
     ( select   
            cfg.technology,
            ...
            cfg.is_active
         from g77_cfg.t_meta_interface_cfg cfg
         inner join g77_cfg.t_meta_interface target
        ...
        union all
         select   
            ...
            cfg.is_active
         from g77_cfg.t_meta_interface_cfg cfg
         where not exists (select 1 from g77_cfg.t_meta_interface target where target.aptitude_project = cfg.aptitude_project )
         ) cfg
  on ( target.aptitude_project = cfg.aptitude_project)
  when matched 
  then 
...
Prompt dropping temporary config table g77_cfg.T_META_INTERFACE_CFG

begin
  execute immediate 'drop table g77_cfg.t_meta_interface_cfg';
end;
/
