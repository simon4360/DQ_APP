...
public class TeradataDatabaseTableOperator implements IDatabaseTableOperator
{
        ...
        public void alterTrigger  ( final String pTriggerOwner , final String pTriggerName, final String pAction ) throws Exception
    {
        ...
        LOG.debug ( "A trigger will be " + pAction + " using the following DDL '" + ALTERDDL + "'" );

        if ( checkIfExists ( pTriggerOwner , pTriggerName ) )
        {
            try
            {
                conn = databaseConnector.getConnection ();
                ...
                LOG.debug ( "The trigger has been successfully " + pAction );
