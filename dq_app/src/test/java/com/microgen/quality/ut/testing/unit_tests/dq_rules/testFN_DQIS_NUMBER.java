...
public class testFN_DQIS_NUMBER {
        private static final Logger LOG = Logger.getLogger(testFN_DQIS_NUMBER.class);
        ...
        private final Path PATH_TO_RESOURCES = Resources.getPathToResource("testing/dq_rules/FN_DQIS_NUMBER");

        final String TestPrefix = "FID6";
        final String aptitudeProject = "g77_src_ady";
        final String aptitudeUtils = "g77_utils";
        final String sessionId = "DQ_FUNCTION6";
	final String use_case_id = "UCN1043";
        ...
        private void updateConfigData( String pUpdateSQL ) throws Exception
        {
         ...
         LOG.debug ( "Updating active config in t_meta_table_dq so only function 6 is run during this test " + TOKEN_OPS.replaceTokensInString ( pUpdateSQL ) );
             try
             {
        ...
        private void cleardownBeforeEachTest() throws Exception {
                ...
                String pUpdateSQL = "update t_meta_table_dq set DQ_ACTIVE_INDICATOR = 'X' where dq_config_id in (select distinct mdq.dq_config_id from t_meta_table_dq mdq inner join v_meta_batch_task_dq dq on dq.dq_config_id = mdq.dq_config_id where dq.dq_aptitude_project = 'g77_src_ady' and mdq.dq_function_id != '6')";
