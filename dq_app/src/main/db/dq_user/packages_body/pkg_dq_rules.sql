...
*******************************************************************************/
PROCEDURE pr_dq_wrapper (  p_table_name         IN    VARCHAR2
...
IS
  ...
  v_process_id        NUMBER         := SQ_DQ_PROCESS_ID.Nextval;
...
BEGIN
  ...
  FOR c IN
     (SELECT config.DQ_CONFIG_ID
           ...
           , config.DQ_MESSAGE
           , config.DQ_TYPE
           , config.DQ_FAILED_DATA_SQL
           , config.DQ_MERGE_SQL
           , config.DQ_COUNT_SQL
           , config.DQ_USE_CASE_ERR_SQL
           , config.DQ_WHERE_SQL
       FROM v_meta_batch_task_dq config
      WHERE config.DQ_STAGING_TABLE      = p_table_name
     )
  LOOP
    ...
    v_update_sql := c.DQ_MERGE_SQL;
    
    -- Get total row count
    v_count_sql  := c.DQ_COUNT_SQL;
    
    EXECUTE IMMEDIATE (v_count_sql) INTO gv_total_row_count;

        pr_dq_run_checks (  p_process_id               =>    v_process_id
...
EXCEPTION
  ...
  ROLLBACK;
  o_return_code := 1;
  pr_error( v_proc_name
		  , gv_error_msg
          , p_source_system    => NULL
          , p_row              => NULL
		  , p_batch_process_id => v_process_id
		  , p_table            => 'V_META_BATCH_TASK_DQ'
		  , p_field            => NULL
          , p_value            => NULL
...
BEGIN
    ...
    THEN
      EXECUTE IMMEDIATE 'UPDATE ' || p_table 
                       || ' SET EVENT_STATUS =
                           CASE WHEN EVENT_STATUS = ' || v_failed_event_status 
                           || ' THEN ' || v_failed_event_status 
                           || ' ELSE ' || v_event_status 
                      || ' END';

/* NEED SOME THOUGHT HERE, HOW DO WE NOT UPDATE THE ENTIRE TABLE?? */
