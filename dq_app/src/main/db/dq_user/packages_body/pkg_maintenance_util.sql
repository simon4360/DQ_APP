CREATE OR REPLACE PACKAGE BODY DQ_USER.pkg_maintenance_util AS
  ...
  is
    ...
    trigger_dml clob := 
  q'[
create or replace trigger DQ_USER.REGEX_TRIGGERNAME
after insert or update or delete
on DQ_USER.REGEX_DRIVINGTABLE referencing old as old new as new
for each row
begin
  case
    when inserting then
      insert into DQ_USER.REGEX_AUDITTABLE (REGEX_COLUMLIST, ACTION, ACTION_TIMESTAMP
...
FUNCTION f_is_view_exists(p_view_name VARCHAR2) return number IS
  ...
  BEGIN
    ...
    MERGE INTO DQ_USER.t_build_history tgt
  ...
  BEGIN
    log_info(p_proc_name => lv_proc_name,
             ...
             p_tagname   => 'version_to',
             p_tag       => p_version_to);
    MERGE INTO DQ_USER.t_build_version
    USING dual
    ON (1 = 1)
    WHEN MATCHED THEN
