create or replace view g77_cfg.v_meta_batch_task_dq_all 
as
select 
  DQ_ACTIVATION
, DQ_ACTIVE_INDICATOR
...
,      'SELECT distinct '|| DQ_STAGING_TABLE || '.GATEWAY_ID'|| chr(10)
     ...
     ||'         FROM g77_cfg.' || DQ_STAGING_TABLE                                                             || chr(10)
     ||                    DQ_WHERE_SQL                                                                         || chr(10)
...
select distinct
           ...
           ||'          AND ' || case when lower(dqc.dq_method) = lower('FUNCTION')
                                      then 'g77_cfg.' || case nvl (dqc.dq_function_parameters, '1') 
                                                             when '1' 
      ...
      from g77_cfg.t_meta_table t
inner join g77_cfg.t_meta_batch_task_table_x btx
        on t.table_id = btx.table_id
inner join g77_cfg.t_meta_batch_task bt
        on btx.batch_task_id = bt.batch_task_id
inner join g77_cfg.t_meta_interface i
        on bt.interface_id = i.interface_id
inner join g77_cfg.t_meta_table_dq dqc
        on dqc.table_id = t.table_id
 left join g77_cfg.t_dq_function func
        on func.dq_function_id   = dqc.dq_function_id
 left join g77_cfg.t_dq_condition cond
        on cond.dq_condition_id  = dqc.dq_condition_id
 left join g77_cfg.t_dq_error_messages err
        on dqc.dq_message_id = err.dq_message_id 
     WHERE 1=1
       and btx.dq_checks <> 'do_not_dq'
       and t.schema_name        = 'G77_CFG'
