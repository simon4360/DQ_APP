create or replace PROCEDURE g77_cfg.pr_meta_validate_config
IS
...
insert into g77_cfg.t_error_log
 (    EVENT_DATETIME
    , ERROR_STATUS
  ...
  (
     with cfg 
              ...
              ( select distinct t.table_name dq_staging_table, t.legal_entity_column dq_legal_entity_column, i.aptitude_project dq_aptitude_project, dq_column_name, dq_function_id
                  from g77_cfg.t_meta_table_dq dq inner join g77_cfg.t_meta_table t on t.table_id = dq.table_id
                    inner
    ...
    (SELECT DISTINCT
     ...
     , cond.DQ_CONDITION
      FROM T_META_TABLE_DQ config
       inner join g77_cfg.t_meta_table t on t.table_id = config.table_id
