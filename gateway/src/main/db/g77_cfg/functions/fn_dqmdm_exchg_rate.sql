CREATE OR REPLACE FUNCTION G77_CFG.FN_DQMDM_EXCHG_RATE (p_session_id IN VARCHAR2, p_rate_type IN VARCHAR2, p_perc NUMBER)
   RETURN VARCHAR2
   DETERMINISTIC
IS
...
BEGIN
  ...
  (
    ...
    SELECT
      ...
      FROM
        G77_CFG.T_MDM_EXCHANGE_RATE_IN
      WHERE
        EVENT_STATUS = 0
  ...
  select 'DQ 1. SOURCE=MV_CUR_EXCHANGE_RATE, MAX_ARRIVAL_TIME='||nvl(to_char(max(ARRIVAL_TIME),'YYYY-MM-DD HH24:MI:SS'),'-')||', CNT='||count(*) into v_message_text_tmp from G77_CFG.T_MDM_EXCHANGE_RATE_IN where SOURCE_DEVICE_NAME = 'MV_CUR_EXCHANGE_RATE' AND EXCHANGE_RATE_TYPE_CODE=p_rate_type AND ACTIVE_INDICATOR='Active';
  v_message_text := v_message_text || ' ' || v_message_text_tmp;
  select 'DQ 2. SOURCE=T_MDM_EXCHANGE_RATE_IN, MAX_ARRIVAL_TIME='||nvl(to_char(max(ARRIVAL_TIME),'YYYY-MM-DD HH24:MI:SS'),'-')||', CNT='||count(*) into v_message_text_tmp from G77_CFG.T_MDM_EXCHANGE_RATE_IN where SOURCE_DEVICE_NAME = 'T_MDM_EXCHANGE_RATE_IN' AND EXCHANGE_RATE_TYPE_CODE=p_rate_type AND ACTIVE_INDICATOR='Active';
  v_message_text := v_message_text || ' ' || v_message_text_tmp;
  select 'DQ 3. SOURCE=MV_CUR_EXCHANGE_RATE, SESSION_ID='||p_session_id||',MAX_ARRIVAL_TIME='||nvl(to_char(max(ARRIVAL_TIME),'YYYY-MM-DD HH24:MI:SS'),'-')||', CNT='||count(*) into v_message_text_tmp from G77_CFG.T_MDM_EXCHANGE_RATE_IN where SOURCE_DEVICE_NAME = 'MV_CUR_EXCHANGE_RATE' AND SESSION_ID=p_session_id AND EXCHANGE_RATE_TYPE_CODE=p_rate_type AND ACTIVE_INDICATOR='Active';
  v_message_text := v_message_text || ' ' || v_message_text_tmp;
  select 'DQ 4. SOURCE=T_MDM_EXCHANGE_RATE_IN, SESSION_ID='||p_session_id||',MAX_ARRIVAL_TIME='||nvl(to_char(max(ARRIVAL_TIME),'YYYY-MM-DD HH24:MI:SS'),'-')||', CNT='||count(*) into v_message_text_tmp from G77_CFG.T_MDM_EXCHANGE_RATE_IN where SOURCE_DEVICE_NAME = 'T_MDM_EXCHANGE_RATE_IN' AND SESSION_ID=p_session_id AND EXCHANGE_RATE_TYPE_CODE=p_rate_type AND ACTIVE_INDICATOR='Active';
  v_message_text := v_message_text || ' ' || v_message_text_tmp;
